...
$(window).load(function(){
	spinner = Rats.UI.LoadAnimation.start();
		...
		<form name="FrmSendSession" id="FrmSendSession" action="http://rs2.portaldwh.bri.co.id" method="post">
			...
			<input type="hidden" id="key" name="key" value="00174122ip=rs3.portaldwh.bri.co.id&addr=ZWR0ekpxMi80bGdwTlBIbVFxMGVjcEsvL2lhWWdWRW5odm5JQnAzTFhwbWxXbFNENU1HaDd0dkw1NnlscFZGaXUrb1Q4R01aREhFcmZ0OW96c2ZFYVE9PQapibrisimrapibrisimrtimbrisim&Kanwil=&Kanca=&Unit=&rs:Command=Render" />
			<input type="hidden" id="client_ip" name="client_ip" value="192.168.67.29" />
	        <input type="hidden" id="reportAddr" name="reportAddr" value="/Monitoring Echannel/INVESTIGASI STO-LAPORAN LOG AKTIVITAS INTERNET BANKING" />
			<input type="hidden" id="reportIp" name="reportIp" value="rs3.portaldwh.bri.co.id" />
			<input type="hidden" id="QueryStr" name="QueryStr" value="ip=rs3.portaldwh.bri.co.id&addr=ZWR0ekpxMi80bGdwTlBIbVFxMGVjcEsvL2lhWWdWRW5odm5JQnAzTFhwbWxXbFNENU1HaDd0dkw1NnlscFZGaXUrb1Q4R01aREhFcmZ0OW96c2ZFYVE9PQapibrisimrapibrisimrtimbrisim&Kanwil=&Kanca=&Unit=0&rs:Command=Render" />
		</form>
		</body>
